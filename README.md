...
- **纯 Python 单体架构**：零数据库依赖，无需 Docker，一个进程跑全部
- **内存占用 < 80MB**（含 WebUI），冷启动 < 1 秒，毫秒级 API 响应
- **工具结果智能压缩**：返回数据经裁剪去噪后再入 LLM 上下文，大幅降低 Token 消耗
- **单文件部署**：Windows 端仅 82KB 的 `deploy.exe`（纯 C 编写，无运行时依赖），一键完成虚拟环境 → 依赖安装 → 启动
- **热更新架构**：配置修改即时生效，切换模型 / API Key / 插件设置无需重启服务
- **上下文预算控制**：工作记忆自动裁剪 + 会话摘要压缩，保证长对话不爆 Token
